...
from pythalab_agent_cli.config.defaults import default_config
from pythalab_agent_cli.core.constants import DEFAULT_TARGET_FILE, MEMORY_DB_NAME, PYTHALAB_DIR
from pythalab_agent_cli.core.types import WorkspacePaths
from pythalab_agent_cli.memory.sqlite_store import SQLiteStore

AGENTS_MD = """# AGENTS.md

This repository is intended to be used with `pythalab-agent-cli`.
...
ALGORITHM_PLACEHOLDER = '''"""Algorithm target file for pythalab-agent-cli.

This file is intentionally left empty. Run ``pythalab-agent run "<task>"`` and the
...
def discover_workspace(start: Path | None = None) -> Path:
    """Discover the workspace root by walking upward to `.pythalab-agent` or git root."""
    current = (start or Path.cwd()).resolve()
    for candidate in [current, *current.parents]:
        if (candidate / PYTHALAB_DIR).exists() or (candidate / ".git").exists():
            return candidate
    return current
...
def workspace_paths(repo_root: Path, target_file: str = DEFAULT_TARGET_FILE) -> WorkspacePaths:
    ...
    pythalab_dir = root / PYTHALAB_DIR
    return WorkspacePaths(
        root=root,
        pythalab_dir=pythalab_dir,
        memory_db=pythalab_dir / MEMORY_DB_NAME,
...
def initialize_workspace(repo_root: Path, *, force: bool = False) -> WorkspacePaths:
    ...
    paths.pythalab_dir.mkdir(parents=True, exist_ok=True)
    (paths.pythalab_dir / "logs").mkdir(exist_ok=True)
