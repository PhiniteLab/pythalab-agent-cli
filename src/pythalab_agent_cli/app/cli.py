...
from pythalab_agent_cli import __version__
from pythalab_agent_cli.agent.direct_loop import MissingPackagePrompt
from pythalab_agent_cli.app.commands import doctor_command, init_command, run_command, validate_command
from pythalab_agent_cli.app.interactive import start_repl
from pythalab_agent_cli.config.loader import dump_config, load_config
from pythalab_agent_cli.core.constants import DEFAULT_MODEL, FALLBACK_MODEL
from pythalab_agent_cli.memory.sqlite_store import SQLiteStore
from pythalab_agent_cli.repo.workspace import discover_workspace
from pythalab_agent_cli.ui.progress import RichAgentObserver
from pythalab_agent_cli.ui.tables import key_value_table
...
def init(
    ...
    """Initialize a pythalab-agent workspace."""
    init_command(path.resolve(), force=force)
    console.print(f"[green]Initialized pythalab-agent workspace at {path.resolve()}[/green]")
...
def memory_list(path: Path = typer.Option(Path.cwd(), "--path", help="Workspace path.")) -> None:
    ...
    store = SQLiteStore(root / ".pythalab-agent" / "memory.sqlite")
...
def memory_clear(
    ...
    if not yes and not typer.confirm("Clear local pythalab-agent memory?"):
        raise typer.Exit()
    root = discover_workspace(path.resolve())
    SQLiteStore(root / ".pythalab-agent" / "memory.sqlite").clear()
