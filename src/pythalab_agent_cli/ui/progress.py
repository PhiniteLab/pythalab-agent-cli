...
from pythalab_agent_cli.agent.observer import AgentProgressEvent
...
class RichAgentObserver:
    ...
    def _begin(self, event: AgentProgressEvent) -> None:
        ...
        self._status = status
        self._stop_tick = threading.Event()
        ticker = threading.Thread(target=self._tick_loop, name="pythalab-agent-progress", daemon=True)
        self._ticker = ticker
        ticker.start()
