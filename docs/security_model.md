...
Model-generated code is first stored as `.json` under `.pythalab-agent/staged/`. Real Python files are modified only after staged validation passes. Failed staged drafts remain inspectable but are not materialized.
