...
`pythalab-agent-cli` separates model, control, repository, validation, memory, sandbox, and UI concerns. This branch uses a staged component architecture for weak/small local models.
...
User CLI
  ...
  -> AgentLoop
      ...
      -> Staged JSON storage under .pythalab-agent/staged/
